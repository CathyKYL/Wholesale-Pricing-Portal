"""
Test 360-day historical Keepa data fetching
"""
import requests
from datetime import datetime, timedelta
from config import settings

def test_360_day_history():
    """Test fetching 360 days of historical data for specific ASINs"""
    
    print("🧪 TESTING 360-DAY HISTORICAL DATA FETCHING")
    print("=" * 60)
    ...
    test_asins = [
        ...
        ('0114850003', 'UK', 3),  # Gold Rush Ranch
        ('4027876040', 'UK', 3),  # Wings of Fire
        ('9124233684', 'UK', 3),  # Dragon Ball
    ...
    for asin, marketplace, domain in test_asins:
        ...
        try:
            ...
            print(f"   Keepa epoch: {keepa_epoch.date()}")
            print(f"   Today: {today}")
            ...
            if best_historical_data:
                ...
                print(f"   📅 360-day history: {best_date_range[0]} → {best_date_range[1]} ({best_historical_count} points)")
                print(f"   Historical points: {best_historical_count} / Future: {len(future_dates) if 'future_dates' in locals() else 0}")
